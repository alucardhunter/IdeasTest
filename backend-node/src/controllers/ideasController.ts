...
export async function createIdea(req: Request, res: Response) {
  ...
  if (!title || !description || !createdBy) return res.status(400).json({ error: 'missing fields' });
...
export async function voteIdea(req: Request, res: Response) {
  ...
  try {
    ...
    if (err?.code === 'P2002') return res.status(400).json({ error: 'User already voted for this idea' });
    console.error(err);
